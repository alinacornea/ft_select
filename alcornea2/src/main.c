...
/*
**	NOTE: In order to search termcap library,
** compile ussing "-ltermcap" or "-ltermlib"
*/
...
**
** 1. Look up the description of the terminal type in use, using tgetent()
**		and tgetent()
** 2. Get the parrameters (keys pressed by the user) using tcgetattr()
** 3. Setting the terminal into nonconincal mode, allows to set how many
**	characters should be read before input is given to the program.
**	(&= ~ICANON) unset a flag using bit mask operation;
**  ICANON - "line editing" for canonical mode, terminal buffers line at a time
** till finds a newline, noncanonical it's controled by MIN and TIME settings;
**  ECHO - controls wheter input is immediately re-echoed as output;
** 4. Set buffer size to 1 byte(in order for read to return) and time
** wait to 0 sec, (VMIN, VTIME)
...
int		tm_set_terminal(t_select *arg)
{
	...
	tputs(tgetstr("ts", NULL), 1, pputchar);
	return (1);
}
